...
export default function Home() {
  ...
  useEffect(() => {
    if (timerRunning && timer > 0) {
      ...
      if (timer <= 60) setTimeColor("text-red-600"); // Change color to red for the last minute
...
<div className="bg-gray-50 text-gray-900 min-h-screen pt-16 flex flex-col lg:flex-row lg:space-x-4">
  ...
  <div className="lg:w-3/4 p-4">
    ...
    {questions.map((q, index) => (
      <div key={index} className="bg-white p-4 rounded-lg mb-4 w-full shadow-md border border-gray-200">
        ...
        <ul className="w-full text-sm font-medium text-gray-900 bg-gray-100 border border-gray-200 rounded-lg mt-2">
          {Object.entries(q.options).map(([key, option]) => (
            <li key={key} className="w-full border-b border-gray-200 last:border-none">
  ...
  {submitted && marks !== null && (
    <div className="bg-white p-4 rounded-lg shadow-lg">
      ...
      <div className="mt-4">
        <div className="text-sm font-medium text-gray-700 mb-2">Accuracy : {accuracy.toFixed(2)} points
        </div>
      ...
      <p className="text-gray-500 mt-2">Test completed on: {testDate}</p>
    </div>
  )}
</div>
